...
def main():
    ...
    ################## 여기서부터는 유사도 검색
    base_feature = np.load('/content/drive/My Drive/Colab Notebooks/Train_for_pca')
    base_feature_label = np.load('/content/drive/My Drive/Colab Notebooks/Train_label_for_pca')
    ...
    vlad_model.load_state_dict(torch.load('/content/drive/My Drive/Colab Notebooks/VLAD_Checkpoint_20200523_total.pth'), strict=True)

    test_out_feature = vlad_model(x_test_data)
    test_out_feature = np.array(test_out_feature)
    test_out_feature = test_out_feature.reshape(-1,1280)
